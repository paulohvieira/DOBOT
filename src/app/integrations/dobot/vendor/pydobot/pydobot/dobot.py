...
class Dobot:
    ...
    def _send_command(self, msg, wait=False):
        ...
        self._expected_queue_idx = expected_idx
        if self.verbose:
    ...
    def _set_ptp_cmd(self, x, y, z, r, mode, wait):
        ...
        # Infelizmente, se response for None, só teremos como referência o comando anterior. :(
        if response:
            expected_idx = struct.unpack_from('L', response.params, 0)[0]
            self._expected_queue_idx = expected_idx
        else:
            print('[DOBOT]: <<<<<<<<< COMANDO NÃO RETORNOU ID DE RESPOSTA >>>>>>>>>')
        return response
    ...
    def move_to(self, x, y, z, r, mode=PTPMode.MOVL_XYZ, wait=False):
    ...
    @property
    def expected_queue_idx(self):
        return self._expected_queue_idx

    def is_running(self):
        idx = self.expected_queue_idx
        if idx is None:
            return False
        return self.current_queue_index() < idx
